...
if (cohort == "older_adults" & investigation_type == "secondary") {
 ...
 df_input <- df_input %>% 
   filter(!is.na(imd_quintile), !is.na(composition_category), !is.na(age_band),
          !is.na(sex), !is.na(has_asthma), !is.na(has_copd),
...
if (too_few_events_mild) {
  ...
  covid_mild_ses_hh_comp_output <- data.frame(
  ...
  covid_mild_ses_hh_comp_output <- glm_poisson(
    df_input, c("imd_quintile", "composition_category"), "covid_primary_inf",
    "time_covid_primary")
...
if (too_few_events_severe) {
  ...
  #create data frame with same columns as model output creates
  covid_severe_ses_hh_comp_output <- data.frame(
    term = "too few events", estimate = NA, std.error = NA,
    statistic = NA, p.value = NA, conf.low = NA, conf.high = NA)
  ...
  #covid by socioeconomic status and household composition
  covid_severe_ses_hh_comp_output <- glm_poisson(
