...
events <- group_specific_events(df_input, c("composition_category"),
...
if (cohort == "infants_subgroup") {
  ...
  if (too_few_events_mild) {
    ...
    covid_mild_hh_comp_output <- data.frame(term = "too few events",
    ...
    covid_mild_hh_comp_output <- tidy(covid_mild_hh_comp, conf.int = TRUE)
  ...
  if (too_few_events_severe) {
    ...
    #create data frame with same columns as model outputs
    covid_severe_hh_comp_output <- data.frame(term = "too few events",
                                              estimate = NA, std.error = NA,
                                              statistic = NA, p.value = NA,
                                              conf.low = NA, conf.high = NA)
    ...
    covid_severe_hh_comp <- glm(covid_secondary_inf ~ composition_category +
                                ...
                                data = df_input, family = poisson)
    covid_severe_hh_comp_output <- tidy(covid_severe_hh_comp, conf.int = TRUE)
  
  }
  
  # #covid mortality by household composition
  # covid_mortality_hh_comp <- glm(covid_mortality_inf ~ composition_category +
  #                                  age_band + sex + rurality_classification +
  #                                  maternal_age + maternal_smoking_status +
  #                                  maternal_drinking + maternal_drug_usage + 
  #                                  maternal_flu_vaccination + 
  ...
  #                                data = df_input, family = poisson)
  # covid_mortality_hh_comp_output <- tidy(covid_mortality_hh_comp, conf.int = TRUE)
...
if (length(args) == 0) {
  ...
  model_outputs %>%
    write_csv(file = paste0(here::here("output", "results", "models",
                            paste0("covid_", investigation_type)), "/", 
                            "covid_hh_comp_model_outputs_", cohort, "_",
