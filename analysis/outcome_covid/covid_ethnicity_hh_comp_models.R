...
if (cohort == "older_adults" & investigation_type == "secondary") {
  ...
  df_input <- df_input %>% 
    filter(!is.na(latest_ethnicity_group), !is.na(composition_category),
           !is.na(age_band), !is.na(sex), !is.na(has_asthma), !is.na(has_copd),
           ...
           !is.na(has_cancer), !is.na(immunosuppressed),
           !is.na(has_sickle_cell), !is.na(smoking_status),
           !is.na(hazardous_drinking), !is.na(drug_usage))
...
if (too_few_events_mild) {
  ...
  covid_mild_ethnicity_hh_comp_output <- data.frame(
  ...
  covid_mild_ethnicity_hh_comp_output <- glm_poisson(
    df_input, c("latest_ethnicity_group", "composition_category"),
    "covid_primary_inf", "time_covid_primary")
...
if (too_few_events_severe) {
  ...
  #create data frame with same columns as model output creates
  covid_severe_ethnicity_hh_comp_output <- data.frame(
    term = "too few events", estimate = NA, std.error = NA,
    statistic = NA, p.value = NA, conf.low = NA, conf.high = NA)
  ...
  #covid by ethnicity and household composition
  covid_severe_ethnicity_hh_comp_output <- glm_poisson(
    df_input, c("latest_ethnicity_group", "composition_category"),
