...
events <- group_specific_events_further(df_input, c("latest_ethnicity_group",
                                        "imd_quintile"), "covid_primary_inf",
                                        "covid_secondary_inf",
...
if (cohort == "infants_subgroup") {
  ...
  if (too_few_events_mild) {
    ...
    covid_mild_ethnicity_ses_further_output <- data.frame(term = "too few events",
                                                          estimate = NA,
                                                          std.error = NA,
                                                          statistic = NA,
                                                          p.value = NA,
    ...
    covid_mild_ethnicity_ses_further_output <- tidy(covid_mild_ethnicity_ses_further, conf.int = TRUE)
  ...
  if (too_few_events_severe) {
    ...
    #create data frame with same columns as model outputs
    covid_severe_ethnicity_ses_further_output <- data.frame(term = "too few events",
                                                            estimate = NA,
                                                            std.error = NA,
                                                            statistic = NA,
                                                            p.value = NA,
    ...
    covid_severe_ethnicity_ses_further <- glm(covid_secondary_inf ~ latest_ethnicity_group +
                                              ...
                                              data = df_input, family = poisson)
    covid_severe_ethnicity_ses_further_output <- tidy(covid_severe_ethnicity_ses_further, conf.int = TRUE)
  ...
  #                                      data = df_input, family = poisson)
  # covid_mortality_ethnicity_ses_further_output <- tidy(covid_mortality_ethnicity_ses_further, conf.int = TRUE)
...
model_outputs_list <- list(covid_mild_ethnicity_ses_further_output, 
                           covid_severe_ethnicity_ses_further_output)#,
                           # covid_mortality_ethnicity_ses_further_output)
...
if (length(args) == 0) {
  ...
  model_outputs %>%
    write_csv(file = paste0(here::here("output", "results", "models",
                            paste0("covid_", investigation_type)), "/", 
                            "further_covid_ethnicity_ses_model_outputs_",
