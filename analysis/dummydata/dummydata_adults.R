...
study_start_date <- study_dates$season1_start_date #change depending on season you want data for
study_end_date <- study_dates$season1_end_date #change depending on season you want data for
...
sim_list = lst(
  ...
  patient_end_day = bn_node(
    ~ as.integer(if_else(runif(n = ..n) < 0.02, 
                 as.integer(runif(n = ..n, patient_index_day + 1, index_day + 365)), 
                 index_day + 365))
  ),
  ...
  #age of the patient
  age = bn_node(
    ~ as.integer(rnormTrunc(n = ..n, mean = 60, sd = 14, min = 18, max = 65))
  ),
  
  #appropriate age for cohort
  is_appropriate_age = bn_node(
    ~ if_else(age >= 18 & age <= 65, TRUE, FALSE)
  ),
...
write_feather(dummydata_processed, sink = here::here("analysis", "dummydata", 
  paste0("dummyextract_adults_", year(study_start_date), "_", year(study_end_date), ".arrow")))
