...
if (cohort == "infants_subgroup") {
  ...
  if (codelist_type == "sensitive") {
    ...
    if (too_few_events_mild) {
      ...
      overall_resp_mild_full_further_output <- data.frame(term = "too few events",
                                                          estimate = NA,
                                                          std.error = NA,
                                                          statistic = NA,
                                                          p.value = NA,
      ...
      overall_resp_mild_full_further <- glm(overall_resp_primary_inf ~ latest_ethnicity_group +
                                              imd_quintile + composition_category +
      ...
      overall_resp_mild_full_further_output <- tidy(overall_resp_mild_full_further, conf.int = TRUE)
    ...
    if (too_few_events_severe) {
      ...
      #create data frame with the same columns as model outputs
      overall_resp_severe_full_further_output <- data.frame(term = "too few events",
                                                            estimate = NA,
                                                            std.error = NA,
                                                            statistic = NA,
                                                            p.value = NA,
      ...
      overall_resp_severe_full_further <- glm(overall_resp_secondary_inf ~ latest_ethnicity_group +
                                                imd_quintile + composition_category +
                                                age_band + sex + rurality_classification +
                                                maternal_age + maternal_smoking_status +
                                                maternal_drinking + maternal_drug_usage +
                                              ...
                                              data = df_input, family = poisson)
      overall_resp_severe_full_further_output <- tidy(overall_resp_severe_full_further, conf.int = TRUE)
      
    }
    
    # #overall_resp mortality by ethnicity, socioeconomic status and household composition
    # overall_resp_mortality_full_further <- glm(overall_resp_mortality_inf ~ latest_ethnicity_group +
    #                                      imd_quintile + composition_category +
    #                                      age_band + sex + rurality_classification +
    #                                      maternal_age + maternal_smoking_status +
    #                                      maternal_drinking + maternal_drug_usage +
    ...
    #                                    data = df_input, family = poisson)
    # overall_resp_mortality_full_further_output <- tidy(overall_resp_mortality_full_further, conf.int = TRUE)
    
  }
  
  # #all cause mortality by ethnicity, socioeconomic status and household composition
  # all_cause_mortality_full_further <- glm(all_cause_mortality_inf ~ latest_ethnicity_group +
  #                                   imd_quintile + composition_category +
  #                                   age_band + sex + rurality_classification +
  #                                   maternal_age + maternal_smoking_status +
  #                                   maternal_drinking + maternal_drug_usage +
  ...
  #                                 data = df_input, family = poisson)
  # all_cause_mortality_full_further_output <- tidy(all_cause_mortality_full_further, conf.int = TRUE)
...
#                    "Overall Respiratory Virus Mortality by Ethnicity, IMD Quintile and Household Composition",
#                    "All Cause Mortality by Ethnicity, IMD Quintile and Household Composition")
# } else {
#   model_names <- c("All Cause Mortality by Ethnicity, IMD Quintile and Household Composition")
# }
...
# model_outputs_list <- list(all_cause_mortality_full_further_output)
# 
# #adjust the model outputs list based on the conditions
# if (codelist_type == "sensitive") {
#   model_outputs_list <- c(model_outputs_list,
#                           list(overall_resp_mild_full_further_output,
#                                overall_resp_severe_full_further_output,
#                                overall_resp_mortality_full_further_output))
# }
