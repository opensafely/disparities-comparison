...
if (cohort == "infants_subgroup") {
  ...
  if (codelist_type == "sensitive") {
    ...
    if (too_few_events_mild) {
      ...
      overall_resp_mild_ethnicity_further_output <- data.frame(term = "too few events",
                                                               estimate = NA,
                                                               std.error = NA,
                                                               statistic = NA,
                                                               p.value = NA,
      ...
      overall_resp_mild_ethnicity_further_output <- tidy(overall_resp_mild_ethnicity_further, conf.int = TRUE)
    ...
    if (too_few_events_severe) {
      ...
      #create data frame with same columns as model outputs
      overall_resp_severe_ethnicity_further_output <- data.frame(term = "too few events",
                                                                 estimate = NA,
                                                                 std.error = NA,
                                                                 statistic = NA,
                                                                 p.value = NA,
      ...
      overall_resp_severe_ethnicity_further <- glm(overall_resp_secondary_inf ~ latest_ethnicity_group +
                                                   ...
                                                   data = df_input, family = poisson)
      overall_resp_severe_ethnicity_further_output <- tidy(overall_resp_severe_ethnicity_further, conf.int = TRUE)
      
    }
    
    # #overall_resp mortality by ethnicity
    # overall_resp_mortality_ethnicity_further <- glm(overall_resp_mortality_inf ~ latest_ethnicity_group +
    ...
    #                                         data = df_input, family = poisson)
    # overall_resp_mortality_ethnicity_further_output <- tidy(overall_resp_mortality_ethnicity_further, conf.int = TRUE)
    
  }
  
  # #all cause mortality by ethnicity
  # all_cause_mortality_ethnicity_further <- glm(all_cause_mortality_inf ~ latest_ethnicity_group +
  #                                        age_band + sex + rurality_classification +
  #                                        maternal_age + maternal_smoking_status +
  #                                        maternal_drinking + maternal_drug_usage +
  #                                        maternal_flu_vaccination +
  ...
  #                                      data = df_input, family = poisson)
  # all_cause_mortality_ethnicity_further_output <- tidy(all_cause_mortality_ethnicity_further, conf.int = TRUE)
...
#                           list(overall_resp_mild_ethnicity_further_output,
#                                overall_resp_severe_ethnicity_further_output,
#                                overall_resp_mortality_ethnicity_further_output))
# }

#create the model outputs list
model_outputs_list <- list(overall_resp_mild_ethnicity_further_output,
                           overall_resp_severe_ethnicity_further_output)
...
if (length(args) == 0) {
  ...
  model_outputs %>%
    write_csv(file = paste0(here::here("output", "results", "models",
                            ...
                            investigation_type)), "/",
                            "further_overall_and_all_cause_ethnicity_model_outputs_",
