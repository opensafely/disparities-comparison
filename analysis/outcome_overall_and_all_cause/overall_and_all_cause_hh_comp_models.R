...
if (too_few_events_mild) {
  ...
  overall_resp_mild_hh_comp_output <- data.frame(
  ...
  overall_resp_mild_hh_comp_output <- glm_poisson(
...
if (too_few_events_severe) {
  ...
  #create data frame with same columns as model output creates
  overall_resp_severe_hh_comp_output <- data.frame(
    term = "too few events", estimate = NA, std.error = NA,
    statistic = NA, p.value = NA, conf.low = NA, conf.high = NA)
  ...
  #overall primary by household composition
  overall_resp_severe_hh_comp_output <- glm_poisson(
    df_input, "composition_category", "overall_resp_secondary_inf",
    "time_overall_resp_secondary")
  
}

#create the model outputs list
model_outputs_list <- list(overall_resp_mild_hh_comp_output,
