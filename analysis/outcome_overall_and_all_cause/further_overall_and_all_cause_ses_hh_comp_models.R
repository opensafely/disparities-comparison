...
if (cohort == "infants_subgroup") {
  ...
  if (codelist_type == "sensitive") {
    ...
    if (too_few_events_mild) {
      ...
      overall_resp_mild_ses_hh_comp_further_output <- data.frame(term = "too few events",
                                                                 estimate = NA,
                                                                 std.error = NA,
                                                                 statistic = NA,
                                                                 p.value = NA,
      ...
      overall_resp_mild_ses_hh_comp_further <- glm(overall_resp_primary_inf ~ imd_quintile +
      ...
      overall_resp_mild_ses_hh_comp_further_output <- tidy(overall_resp_mild_ses_hh_comp_further, conf.int = TRUE)
    ...
    if (too_few_events_severe) {
      ...
      #create data frame with the same columns as model outputs
      overall_resp_severe_ses_hh_comp_further_output <- data.frame(term = "too few events",
                                                                   estimate = NA,
                                                                   std.error = NA,
                                                                   statistic = NA,
                                                                   p.value = NA,
      ...
      overall_resp_severe_ses_hh_comp_further <- glm(overall_resp_secondary_inf ~ imd_quintile +
                                                       composition_category + age_band +
                                                       sex + rurality_classification +
                                                       maternal_age + maternal_smoking_status +
                                                       maternal_drinking + maternal_drug_usage +
                                                     ...
                                                     data = df_input, family = poisson)
      overall_resp_severe_ses_hh_comp_further_output <- tidy(overall_resp_severe_ses_hh_comp_further, conf.int = TRUE)
      
    }
    
    # #overall_resp mortality by socioeconomic status and household composition
    # overall_resp_mortality_ses_hh_comp_further <- glm(overall_resp_mortality_inf ~ imd_quintile +
    #                                             composition_category + age_band +
    #                                             sex + rurality_classification +
    #                                             maternal_age + maternal_smoking_status +
    #                                             maternal_drinking + maternal_drug_usage +
    ...
    #                                           data = df_input, family = poisson)
    # overall_resp_mortality_ses_hh_comp_further_output <- tidy(overall_resp_mortality_ses_hh_comp_further, conf.int = TRUE)
    
  }
  
  # #all cause mortality by socioeconomic status and household composition
  # all_cause_mortality_ses_hh_comp_further <- glm(all_cause_mortality_inf ~ imd_quintile +
  #                                          composition_category + age_band +
  #                                          sex + rurality_classification +
  #                                          maternal_age + maternal_smoking_status +
  #                                          maternal_drinking + maternal_drug_usage +
  ...
  #                                        data = df_input, family = poisson)
  # all_cause_mortality_ses_hh_comp_further_output <- tidy(all_cause_mortality_ses_hh_comp_further, conf.int = TRUE)
...
# if (codelist_type == "sensitive") {
#   model_names <- c("Mild Overall Respiratory Virus by IMD Quintile and Household Composition",
#                    "Severe Overall Respiratory Virus by IMD Quintile and Household Composition",
#                    "Overall Respiratory Virus Mortality by IMD Quintile and Household Composition",
#                    "All Cause Mortality by IMD Quintile and Household Composition")
# } else {
#   model_names <- c("All Cause Mortality by IMD Quintile and Household Composition")
# }
...
# model_outputs_list <- list(all_cause_mortality_ses_hh_comp_further_output)
# 
# #adjust the model outputs list based on the conditions
# if (codelist_type == "sensitive") {
#   model_outputs_list <- c(model_outputs_list,
#                           list(overall_resp_mild_ses_hh_comp_further_output,
#                                overall_resp_severe_ses_hh_comp_further_output,
#                                overall_resp_mortality_ses_hh_comp_further_output))
# }

#create the model outputs list
model_outputs_list <- list(overall_resp_mild_ses_hh_comp_further_output,
                           overall_resp_severe_ses_hh_comp_further_output)
...
if (length(args) == 0) {
  ...
  model_outputs %>%
    write_csv(file = paste0(here::here("output", "results", "models",
                            paste0("overall_and_all_cause_",
                            investigation_type)), "/",
                            "further_overalL_and_all_cause_ses_hh_comp_model_outputs_",
