...
if (cohort == "infants_subgroup") {
  ...
  if (codelist_type == "sensitive") {
    ...
    if (too_few_events_milds) {
      ...
      overall_resp_mild_hh_comp_further_output <- data.frame(term = "too few events",
                                                             estimate = NA,
                                                             std.error = NA,
                                                             statistic = NA,
                                                             p.value = NA,
      ...
      overall_resp_mild_hh_comp_further_output <- tidy(overall_resp_mild_hh_comp_further, conf.int = TRUE)
    ...
    if (too_few_events_severe) {
      ...
      #create data frame with the same columns as model outputs
      overall_resp_severe_hh_comp_further_output <- data.frame(term = "too few events",
                                                               estimate = NA,
                                                               std.error = NA,
                                                               statistic = NA,
                                                               p.value = NA,
      ...
      overall_resp_severe_hh_comp_further <- glm(overall_resp_secondary_inf ~ composition_category +
                                                 ...
                                                 data = df_input, family = poisson)
      overall_resp_severe_hh_comp_further_output <- tidy(overall_resp_severe_hh_comp_further, conf.int = TRUE)
      
    }
    
    # #overall_resp mortality by household composition
    # overall_resp_mortality_hh_comp_further <- glm(overall_resp_mortality_inf ~ composition_category +
    #                                         age_band + sex + rurality_classification +
    #                                         maternal_age + maternal_smoking_status +
    #                                         maternal_drinking + maternal_drug_usage +
    #                                         maternal_flu_vaccination +
    ...
    #                                       data = df_input, family = poisson)
    # overall_resp_mortality_hh_comp_further_output <- tidy(overall_resp_mortality_hh_comp_further, conf.int = TRUE)
    
  }
  
  # #all cause mortality by household composition
  # all_cause_mortality_hh_comp_further <- glm(all_cause_mortality_inf ~ composition_category +
  #                                      age_band + sex + rurality_classification +
  #                                      maternal_age + maternal_smoking_status +
  #                                      maternal_drinking + maternal_drug_usage +
  #                                      maternal_flu_vaccination +
  ...
  #                                    data = df_input, family = poisson)
  # all_cause_mortality_hh_comp_further_output <- tidy(all_cause_mortality_hh_comp_further, conf.int = TRUE)
...
#                           list(overall_resp_mild_hh_comp_further_output,
