...
ethnicity_groups <- as.numeric(length(unique(results_ethnicity$latest_ethnicity_group)))
if (study_start_date >= covid_season_min) {
  final_results <- rbind(
    final_results,
    data.frame(
...
#calculate total number of events for each outcome type by household composition type
if (study_start_date >= covid_season_min) {
...
#add this to final results with 'Group' as IMD Quintile 
hh_comp_groups <- as.numeric(length(unique(results_hh_comp$composition_category)))
if (study_start_date >= covid_season_min) {
  final_results <- rbind(
    final_results,
    data.frame(
      ...
      Characteristic = rep("Household Composition Category", 10 * hh_comp_groups),
  ...
  final_results <- rbind(
    ...
    data.frame(
      ...
      Events = results_hh_comp$events,
      Rate = results_hh_comp$incidence_rate,
      Characteristic = rep("Household Composition Category", 13 * hh_comp_groups),
      Group = results_hh_comp$composition_category)
  )
} else {
  final_results <- rbind(
    ...
    data.frame(
      ...
      Events = results_hh_comp$events,
      Rate = results_hh_comp$incidence_rate,
      Characteristic = rep("Household Composition Category", 7 * hh_comp_groups),
      Group = results_hh_comp$composition_category)
  )
}
...
#calculate total number of events for each outcome type by rurality classification
if (study_start_date >= covid_season_min) {
...
rurality_groups <- as.numeric(length(unique(results_rurality$rurality_classification)))
if (study_start_date >= covid_season_min) {
  final_results <- rbind(
    final_results,
    data.frame(
...
if (cohort == "children_and_adolescents" | cohort == "adults" | cohort == "older_adults") {
  if (study_start_date >= covid_prior_vacc_min) {
    ...
    final_results <- rbind(
      final_results,
      data.frame(
        Outcome = c(rep("COVID mild", cov_vaccines_groups), 
                    rep("COVID mortality", cov_vaccines_groups),
                    rep("COVID severe", cov_vaccines_groups)),
    ...
    survival_cov_vaccines <- df_input %>%
      group_by(covid_vaccination_mild) %>%
      transmute(
        covid_mild = sum(time_covid_primary, na.rm = T)
      ) %>%
      ungroup() %>%
      group_by(covid_vaccination_severe) %>%
      transmute(
        covid_severe = sum(time_covid_secondary, na.rm = T),
      ) %>%
      ungroup() %>%
  ...
  final_results <- rbind(
    final_results,
    data.frame(
      Outcome = c(rep("Flu mild", 2), rep("Flu mortality", 2),
                  rep("Flu severe", 2)),
        PYears = results_prior_flu_vacc$person_years,
        Events = results_prior_flu_vacc$events,
        Rate = results_prior_flu_vacc$incidence_rate,
        Characteristic = rep("Vaccinated against influenza in previous season", 6),
        Group = results_prior_flu_vacc$prior_flu_vaccination)
    )
  
  #calculate total person-time for each outcome type by flu vaccination status
  survival_flu_vacc <- df_input %>%
    group_by(flu_vaccination_mild) %>%
    transmute(
      flu_mild = sum(time_flu_primary, na.rm = T)
      ) %>%
    ungroup() %>%
    group_by(flu_vaccination_severe) %>%
    transmute(
      flu_severe = sum(time_flu_secondary, na.rm = T)
      ) %>%
    ungroup() %>%
  ...
  #get unique rows
  survival_flu_vacc <- unique(survival_flu_vacc)
  
  #reshape
  survival_flu_vacc <- survival_flu_vacc %>%
    pivot_longer(
  ...
  #get unique rows
  events_flu_vacc <- unique(events_flu_vacc)
  
  #reshape
  events_flu_vacc <- events_flu_vacc %>%
    pivot_longer(
      cols = !flu_vaccination,
      names_to = "outcome",
      values_to = "events"
