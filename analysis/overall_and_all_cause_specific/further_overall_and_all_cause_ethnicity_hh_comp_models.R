...
#} else {
  ...
  if (codelist_type == "sensitive") {
    ...
    overall_resp_mild_ethnicity_hh_comp_further_output <- tidy(overall_resp_mild_ethnicity_hh_comp_further)
    ...
    overall_resp_severe_ethnicity_hh_comp_further <- glm(overall_resp_secondary_inf ~ latest_ethnicity_group +
                                                         ...
                                                         data = df_input, family = poisson)
    overall_resp_severe_ethnicity_hh_comp_further_output <- tidy(overall_resp_severe_ethnicity_hh_comp_further)
    
    #overall_resp mortality by ethnicity and household composition
    overall_resp_mortality_ethnicity_hh_comp_further <- glm(overall_resp_mortality ~ latest_ethnicity_group + 
                                                              composition_category + 
                                                              age_band + sex + 
                                                              rurality_classification + 
                                                              prior_flu_vaccination +
                                                            ...
                                                            data = df_input, family = poisson)
    overall_resp_mortality_ethnicity_hh_comp_further_output <- tidy(overall_resp_mortality_ethnicity_hh_comp_further)
  }
  
  #all cause mortality by ethnicity and household composition
  all_cause_mortality_ethnicity_hh_comp_further <- glm(all_cause_mortality ~ latest_ethnicity_group +
                                                         composition_category + 
                                                         age_band + sex + 
                                                         rurality_classification + 
                                                         prior_flu_vaccination +
                                                       ...
                                                       data = df_input, family = poisson)
  all_cause_mortality_ethnicity_hh_comp_further_output <- tidy(all_cause_mortality_ethnicity_hh_comp_further)
...
if (codelist_type == "sensitive") {
  model_outputs_list <- c(model_outputs_list, list(overall_resp_mild_ethnicity_hh_comp_further_output,
