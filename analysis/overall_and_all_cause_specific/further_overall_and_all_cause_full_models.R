...
#} else {
  ...
  if (codelist_type == "sensitive") {
    ...
    overall_resp_mild_full_further_output <- tidy(overall_resp_mild_full_further)
    ...
    overall_resp_severe_full_further <- glm(overall_resp_secondary_inf ~ latest_ethnicity_group +
                                            ...
                                            data = df_input, family = poisson)
    overall_resp_severe_full_further_output <- tidy(overall_resp_severe_full_further)
    
    #overall_resp mortality by ethnicity, socioeconomic status and household composition
    overall_resp_mortality_full_further <- glm(overall_resp_mortality ~ latest_ethnicity_group + 
                                                 imd_quintile + composition_category +
                                                 age_band + sex + rurality_classification + 
                                                 prior_flu_vaccination +
                                                 flu_vaccination +
                                               ...
                                               data = df_input, family = poisson)
    overall_resp_mortality_full_further_output <- tidy(overall_resp_mortality_full_further)
  }
  
  #all cause mortality by ethnicity, socioeconomic status and household composition
  all_cause_mortality_full_further <- glm(all_cause_mortality ~ latest_ethnicity_group +
                                            imd_quintile + composition_category +
                                            age_band + sex + rurality_classification +
                                            prior_flu_vaccination +
                                            flu_vaccination +
                                          ...
                                          data = df_input, family = poisson)
  all_cause_mortality_full_further_output <- tidy(all_cause_mortality_full_further)
...
if (study_start_date < covid_season_min) {
  model_names <- c("All Cause Mortlality by Ethnicity, IMD Quintile and Household Composition")
...
if (codelist_type == "sensitive") {
  model_outputs_list <- c(model_outputs_list, list(overall_resp_mild_full_further_output,
