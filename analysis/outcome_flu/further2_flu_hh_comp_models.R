...
if (too_few_events_mild) {
  ...
  flu_mild_hh_comp_further_output <- data.frame(
  ...
  flu_mild_hh_comp_further_output <- glm_poisson_further(
    df_input, "composition_category", "flu_primary_inf",
    prior_vacc = "prior_flu_vaccination", vacc_mild = NULL,
    offset_var = "time_flu_primary")
...
if (too_few_events_severe) {
  ...
  #create data frame with same columns as model output creates
  flu_severe_hh_comp_further_output <- data.frame(
    term = "too few events", estimate = NA, std.error = NA,
    statistic = NA, p.value = NA, conf.low = NA, conf.high = NA)
  ...
  #flu by household composition
  flu_severe_hh_comp_further_output <- glm_poisson_further(
    df_input, "composition_category", "flu_secondary_inf",
    prior_vacc = "prior_flu_vaccination", vacc_severe = NULL,
    offset_var = "time_flu_secondary")
...
if (length(args) == 0) {
  ...
  model_outputs %>%
    write_csv(file = paste0(here::here("output", "results", "models",
                            paste0("flu_", investigation_type)), "/", 
                            "further2_flu_hh_comp_model_outputs_", cohort,
