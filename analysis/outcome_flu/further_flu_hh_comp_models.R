...
if (too_few_events_mild) {
  ...
  flu_mild_hh_comp_further_output <- data.frame(term = "too few events",
  ...
  flu_mild_hh_comp_further_output <- tidy(flu_mild_hh_comp_further, conf.int = TRUE)
...
if (too_few_events_severe) {
  ...
  #create data frame with same columns as model output creates
  flu_severe_hh_comp_further_output <- data.frame(term = "too few events",
                                                  estimate = NA, std.error = NA,
                                                  statistic = NA, p.value = NA,
                                                  conf.low = NA, conf.high = NA)
  ...
  flu_severe_hh_comp_further <- glm(flu_secondary_inf ~ composition_category + 
                                    ...
                                    data = df_input, family = poisson)
  flu_severe_hh_comp_further_output <- tidy(flu_severe_hh_comp_further, conf.int = TRUE)
...
#                                      data = df_input, family = poisson)
# flu_mortality_hh_comp_further_output <- tidy(flu_mortality_hh_comp_further, conf.int = TRUE)
...
if (length(args) == 0) {
  ...
  model_outputs %>%
    write_csv(file = paste0(here::here("output", "results", "models",
                            paste0("flu_", investigation_type)), "/", 
                            "further_flu_hh_comp_model_outputs_", cohort,
