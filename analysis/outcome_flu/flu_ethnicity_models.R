...
if (cohort == "older_adults" & investigation_type == "secondary") {
  ...
  df_input <- df_input %>% 
    filter(!is.na(latest_ethnicity_group), !is.na(age_band), !is.na(sex),
           ...
           !is.na(has_other_resp), !is.na(has_diabetes),
           !is.na(has_addisons), !is.na(severe_obesity), !is.na(has_chd),
           !is.na(has_ckd), !is.na(has_cld), !is.na(has_cnd),
...
events <- group_specific_events(
  df_input, c("latest_ethnicity_group"), "flu_primary_inf",
  "flu_secondary_inf")
...
if (too_few_events_mild | too_few_events_severe) print(events, n = Inf)
...
if (length(args) == 0) {
  ...
  model_outputs %>%
    write_csv(file = paste0(here::here("output", "results", "models",
                            paste0("flu_", investigation_type)), "/", 
                            "flu_ethnicity_model_outputs_", cohort, "_",
