...
# Create intervals based on study dates 
intervals <- tibble(start_date = seq(study_start_date, study_end_date,
                                     by = "week")) %>%
...
# Define the function to calculate weekly rates per 1000 person-years
calculate_weekly_rates <- function(df, outcomes, group) {
  ...
  bind_rows(lapply(outcomes, function(outcome) {
    ...
    df <- df %>% 
      group_by(week_beginning = start_date, group = as.character(.data[[group]])) %>%
      summarise(
      ...
      right_join(intervals %>% select(start_date), by = c("week_beginning" = "start_date")) %>%
      mutate(
      ...
      select(week_beginning, outcome, characteristic, group, events_midpoint10,
             total_person_years, num_people_midpoint10, rate_pys_midpoint10_derived,
...
pathogens <- case_when(
  ...
  study_start_date >= covid_season_min ~ list(c("rsv", "flu", "covid",
     "all_cause")),
  codelist_type == "sensitive" ~ list(c("rsv", "flu", "overall_resp", "all_cause")),
...
for (i in 1:length(characteristics)) {
  ...
  rates <- calculate_weekly_rates(df_rsv, outcomes_rsv, char)
...
for (i in 1:length(characteristics)) {
  ...
  # Calculate rolling rates for each outcome and characteristic
  rates <- calculate_weekly_rates(df_flu, outcomes_flu, char)
...
if (study_start_date >= covid_season_min) {
  ...
  for (i in 1:length(characteristics)) {
    ...
    rates <- calculate_weekly_rates(df_covid, outcomes_covid, char)
...
if (codelist_type == "sensitive") {
  ...
  for (i in 1:length(characteristics)) {
    ...
    # Calculate rolling rates for each outcome and characteristic
    rates <- calculate_weekly_rates(df_overall_resp, outcomes_overall_resp, char)
...
for (i in 1:length(characteristics)) {
  ...
  # Calculate rolling rates for each outcome and characteristic
  rates <- calculate_weekly_rates(df_all_cause, outcomes_all_cause, char)
