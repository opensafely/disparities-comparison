...
if (too_few_events_mild) {
  ...
  rsv_mild_hh_comp_output <- data.frame(
  ...
  rsv_mild_hh_comp_output <- glm_poisson(
    df_input, "composition_category", "rsv_primary_inf", "time_rsv_primary")
...
if (too_few_events_severe) {
  ...
  #create data frame with same columns as model output creates
  rsv_severe_hh_comp_output <- data.frame(
    term = "too few events", estimate = NA, std.error = NA,
    statistic = NA, p.value = NA, conf.low = NA, conf.high = NA)
  ...
  #rsv primary by household composition
  rsv_severe_hh_comp_output <- glm_poisson(
    df_input, "composition_category", "rsv_secondary_inf", "time_rsv_secondary")
...
model_outputs_list <- list(rsv_mild_hh_comp_output,
                           rsv_severe_hh_comp_output)
...
if (length(args) == 0) {
  ...
  model_outputs %>%
    write_csv(file = paste0(here::here("output", "results", "models",
                            paste0("rsv_", investigation_type)), "/", 
                            "rsv_hh_comp_model_outputs_", cohort, "_",
