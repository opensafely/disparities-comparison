...
events <- group_specific_events_further_(df_input, c("latest_ethnicity_group",
...
if (cohort == "infants_subgroup") {
  ...
  if (too_few_events_mild) {
    ...
    rsv_mild_full_further_output <- data.frame(term = "too few events",
    ...
    rsv_mild_full_further <- glm(rsv_primary_inf ~ latest_ethnicity_group +
                                   imd_quintile + composition_category +
    ...
    rsv_mild_full_further_output <- tidy(rsv_mild_full_further, conf.int = TRUE)
  ...
  if (too_few_events_mild) {
    ...
    #create data frame with same columns as model output creates
    rsv_severe_full_further_output <- data.frame(term = "too few events",
                                                 estimate = NA, std.error = NA,
                                                 statistic = NA, p.value = NA,
                                                 conf.low = NA, conf.high = NA)
    ...
    rsv_severe_full_further <- glm(rsv_secondary_inf ~ latest_ethnicity_group +
                                     imd_quintile + composition_category +
                                     age_band + sex + rurality_classification +
                                     maternal_age + maternal_smoking_status +
                                     maternal_drinking + maternal_drug_usage +
                                   ...
                                   data = df_input, family = poisson)
    rsv_severe_full_further_output <- tidy(rsv_severe_full_further, conf.int = TRUE)
    
  }
  
  # #rsv mortality by ethnicity, socioeconomic status and household composition
  # rsv_mortality_full_further <- glm(rsv_mortality ~ latest_ethnicity_group + 
  ...
  #                           data = df_input, family = poisson)
  # rsv_mortality_full_further_output <- tidy(rsv_mortality_full_further, conf.int = TRUE)
...
model_outputs_list <- list(rsv_mild_full_further_output,
                           rsv_severe_full_further_output)#,
# rsv_mortality_full_further_output)
...
if (length(args) == 0) {
  ...
  model_outputs %>%
    write_csv(file = paste0(here::here("output", "results", "models",
                            paste0("rsv_", investigation_type)), "/", 
                            "further_rsv_full_model_outputs_", cohort, "_", 
