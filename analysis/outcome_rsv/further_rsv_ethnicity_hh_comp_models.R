...
if (cohort == "infants_subgroup") {
  ...
  if (too_few_events_mild) {
    ...
    rsv_mild_ethinicty_hh_comp_output <- data.frame(term = "too few events",
    ...
    rsv_mild_ethnicity_hh_comp_further_output <- tidy(rsv_mild_ethnicity_hh_comp_further, conf.int = TRUE)
  ...
  if (too_few_events_mild) {
    ...
    #create data frame with same columns as model output creates
    rsv_severe_ethnicity_hh_comp_further_output <- data.frame(term = "too few events",
                                                              estimate = NA,
                                                              std.error = NA,
                                                              statistic = NA,
                                                              p.value = NA,
    ...
    rsv_severe_ethnicity_hh_comp_further <- glm(rsv_secondary_inf ~ latest_ethnicity_group +
                                                ...
                                                data = df_input, family = poisson)
    rsv_severe_ethnicity_hh_comp_further_output <- tidy(rsv_severe_ethnicity_hh_comp_further, conf.int = TRUE)
    
  }
  
  # #rsv mortality by ethnicity and household composition
  # rsv_mortality_ethnicity_hh_comp_further <- glm(rsv_mortality_inf ~ latest_ethnicity_group + 
  ...
  #                                        data = df_input, family = poisson)
  # rsv_mortality_ethnicity_hh_comp_further_output <- tidy(rsv_mortality_ethnicity_hh_comp_further, conf.int = TRUE)
  ...
  if (too_few_events_mild) {
    ...
    #create data frame with same columns as model output creates
    rsv_mild_ethnicity_hh_comp_further_output <- data.frame(term = "too few events",
                                                            estimate = NA,
                                                            std.error = NA,
                                                            statistic = NA,
                                                            p.value = NA,
