...
#     rsv_mild_ethnicity_hh_comp_output <- tidy(rsv_mild_ethnicity_hh_comp, conf.int = TRUE)
#   
#   }
#   
#   if (too_few_events_mild) {
#  
#     #create data frame with same columns as model output creates
#     rsv_severe_ethnicity_hh_comp_output <- data.frame(term = "too few events",
#                                                       estimate = NA, std.error = NA,
#                                                       statistic = NA, p.value = NA,
#                                                       conf.low = NA, conf.high = NA)
#   
...
#                                         data = df_input, family = poisson)
#     rsv_severe_ethnicity_hh_comp_output <- tidy(rsv_severe_ethnicity_hh_comp, conf.int = TRUE)
#   
#   }
#   
#   # #rsv mortality by ethnicity and household composition
#   # rsv_mortality_ethnicity_hh_comp <- glm(rsv_mortality_inf ~ latest_ethnicity_group + 
#   #                                          composition_category + age_band + sex + 
#   #                                          #rurality_classification + 
#   #                                          maternal_age + maternal_smoking_status +
#   #                                          maternal_drinking + maternal_drug_usage + 
...
#   #                                        data = df_input, family = poisson)
#   # rsv_mortality_ethnicity_hh_comp_output <- tidy(rsv_mortality_ethnicity_hh_comp, conf.int = TRUE)
#   
# } else {
  ...
  if (too_few_events_mild) {
    ...
    #create data frame with same columns as model output creates
    rsv_mild_ethnicity_hh_comp_output <- data.frame(term = "too few events",
                                                    estimate = NA, std.error = NA,
                                                    statistic = NA, p.value = NA,
                                                    conf.low = NA, conf.high = NA)
