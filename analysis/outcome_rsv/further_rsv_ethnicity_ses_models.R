...
#} else {
  ...
  rsv_mild_ethnicity_ses_further_output <- tidy(rsv_mild_ethnicity_ses_further)
  ...
  rsv_severe_ethnicity_ses_further <- glm(rsv_secondary_inf ~ latest_ethnicity_group +
                                  ...
                                  data = df_input, family = poisson)
  rsv_severe_ethnicity_ses_further_output <- tidy(rsv_severe_ethnicity_ses_further)
  
  #rsv mortality by ethnicity and socioeconomic status
  rsv_mortality_ethnicity_ses_further <- glm(rsv_mortality ~ latest_ethnicity_group + 
                                       imd_quintile + age_band + sex + 
                                       rurality_classification + 
                                       prior_flu_vaccination +
                                       flu_vaccination +
                                     ...
                                     data = df_input, family = poisson)
  rsv_mortality_ethnicity_ses_further_output <- tidy(rsv_mortality_ethnicity_ses_further)
...
if (length(args) == 0) {
  model_outputs %>%
    write_csv(file = paste0(here::here("output", "results", "models",
                            ...
                            "futher_rsv_ethnicity_ses_model_outputs_", cohort, "_", 
  ...
  model_outputs %>%
    write_csv(path = paste0(here::here("output", "results", "models",
                            paste0("rsv_", investigation_type)), "/", 
                            "further_rsv_ethnicity_ses_model_outputs_", cohort, "_", 
                            year(study_start_date), "_", year(study_end_date), 
                            "_", codelist_type, "_", investigation_type, ".csv"))
}
