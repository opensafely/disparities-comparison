...
if (cohort == "infants_subgroup") {
  ...
  if (too_few_events_mild) {
    ...
    rsv_mild_ses_hh_comp_output <- data.frame(term = "too few events", estimate = NA,
    ...
    rsv_mild_ses_hh_comp_output <- tidy(rsv_mild_ses_hh_comp, conf.int = TRUE)
  ...
  if (too_few_events_severe) {
    ...
    #create data frame with same columns as model output creates
    rsv_severe_ses_hh_comp_output <- data.frame(term = "too few events", estimate = NA,
                                                std.error = NA, statistic = NA,
                                                p.value = NA, conf.low = NA,
                                                conf.high = NA)
    ...
    rsv_severe_ses_hh_comp <- glm(rsv_secondary_inf ~ imd_quintile +
                                  ...
                                  data = df_input, family = poisson)
    rsv_severe_ses_hh_comp_output <- tidy(rsv_severe_ses_hh_comp, conf.int = TRUE)
  ...
  #                                  data = df_input, family = poisson)
  # rsv_mortality_ses_hh_comp_output <- tidy(rsv_mortality_ses_hh_comp, conf.int = TRUE)
...
model_outputs_list <- list(rsv_mild_ses_hh_comp_output, 
                           rsv_severe_ses_hh_comp_output)#,
                           # rsv_mortality_ses_hh_comp_output)
...
if (length(args) == 0) {
  ...
  model_outputs %>%
    write_csv(file = paste0(here::here("output", "results", "models",
                            paste0("rsv_", investigation_type)), "/", 
                            "rsv_ses_hh_comp_model_outputs_", cohort, "_", 
