...
if (too_few_events_mild) {
  ...
  rsv_mild_ses_further_output <- data.frame(
  ...
  rsv_mild_ses_further_output <- glm_poisson_further(
    df_input, "imd_quintile", "rsv_primary_inf",
...
if (too_few_events_severe) {
  ...
  #create data frame with same columns as model output creates
  rsv_severe_ses_further_output <- data.frame(
    term = "too few events", estimate = NA, std.error = NA,
    statistic = NA, p.value = NA, conf.low = NA, conf.high = NA)
  ...
  #rsv primary by socioeconomic status
  rsv_severe_ses_further_output <- glm_poisson_further(
    df_input, "imd_quintile", "rsv_secondary_inf",
