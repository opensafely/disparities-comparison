...
fs::dir_create(here("post_check", "exploratory_analyses", "plots"))
...
bar_stack <- function(input, season, outcome) {
  ...
  safe_pull <- function(df, codelist, combo_spec) {
    ...
    if (count == "<=10") return(0) else return(as.numeric(count))
  ...
  ggsave(diag2, filename = paste0(here::here("post_check", "exploratory_analyses",
                                  "plots"), "/", cohort, "_stacked_bar_diag_",
                                  season, "_", outcome, ".png"), scale = 1,
         device = "png")
...
df_input <- df_input %>% 
  mutate(
    n = if_else(n != "<=10" & codelist_type == "sensitive", 
                1.5 * as.numeric(n), as.numeric(n))
  )
