...
all <- all %>%
  subset(month >= as.Date("2016-09-01"))

##plot

# ggplot(data = all) + geom_line(aes(x = month, y = total_flu, col = "Flu")) +
#   geom_line(aes(x = month, y = total_rsv, col = "RSV")) + 
#   scale_x_date(date_breaks = "2 years", date_labels = "%Y") + 
#   labs(x = "Year", y = "Monthly Infections", col = "Disease")
# ggplot(data = covid) + 
#   geom_line(aes(x = month, y = total_covid), col = "orange") +
#   labs(x = "Year", y = "Monthly Infections")

rects <- tibble(
  xmin = seq(as.Date("2016-11-01"), as.Date("2022-11-01"), by = "year"),
  xmax = seq(as.Date("2017-03-01"), as.Date("2023-03-01"), by = "year"),
  ymin = 0,
  ymax = Inf
)
...
ggsave(here::here("post_check", "plots", "exploratory_analyses",
                  "seasonality.png"), width = 15, height = 12)
