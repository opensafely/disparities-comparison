  ...
  df <- read_csv(here::here("post_check", "output", "results", "rates",
    ...
    mutate(
      group = case_when(
        ...
        group == "Other Ethnic Groups" ~ "Chinese or Other",
        TRUE ~ group
      )
    )
...
#helper function to create and save plots
create_rolling_plots_multi <- function(season, phenotype) {
  ...
  plot <- df %>%
    ggplot(aes(x = interval_start, y = rate_1000_py_midpoint10_derived,
               group = group2, col = group2)) +
    # geom_line(stat = "identity", linewidth = 1, alpha = alpha_type) +
    geom_smooth(linewidth = 1, alpha = alpha_type, se = FALSE, span = 0.25) +
    ggh4x::facet_grid2(outcome~group1, scales = "free_y", independent = "y") +
    scale_color_viridis_d() + theme_bw() + #ylim(0, Inf) +
    labs(x = paste0(start, "-", end), y = "", col = "Group")
...
#helper function to create and save plots
create_rolling_plots_overall_counts <- function(df) {
  ...
  df <- df %>%
    mutate(
      ...
      days_since_season_start = difftime(month, season_start,
                                         units = "days") + 1,
  ...
  plot_mild <- df %>%
    filter(str_detect(outcome, "Mild")) %>%
    ggplot(aes(x = days_since_season_start,
               y = n, col = subset, alpha = codelist_type)) +
    geom_line(stat = "identity", linewidth = 1) +  theme_bw() +
    scale_color_manual(values = cols) +
    scale_alpha_manual(values = alph) +
    labs(x = "", y = "", col = "Season", alpha = "Phenotype",
         subtitle = paste0("Mild ", pathogen_print)) +
    facet_wrap(~ codelist_type, ncol = 2, scales = "free_y") +
    scale_x_continuous(
      breaks = cumsum(c(0, 30, 31, 30, 31, 28, 31, 30, 31, 30, 31, 31)) - 15,
      labels = c("09", "10", "11", "12", "01", "02", "03",
                 "04", "05", "06", "07", "08")) +
